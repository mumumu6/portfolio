...
for (const [index, comment] of value.comments.entries()) {
  ...
  assert(
    comment?.target && typeof comment.target === 'object',
    `${location}.target is required.`,
  )
  assert(
    targetKinds.has(comment.target.kind),
    `${location}.target.kind is invalid.`,
  )
  assert(
    typeof comment.target.id === 'string' && comment.target.id.length > 0,
    `${location}.target.id is required.`,
  )
  assert(
    Array.isArray(comment.replies) && comment.replies.length > 0,
    `${location}.replies must not be empty.`,
  )
...
for (const [index, thought] of value.thoughts.entries()) {
  ...
  assert(
    /^\d{4}-\d{2}-\d{2}$/.test(thought.date),
    `${location}.date is invalid.`,
  )
  assert(
    /^\d{4}\.\d{2}\.\d{2}$/.test(thought.dateLabel),
    `${location}.dateLabel is invalid.`,
  )
  assert(
    typeof thought.body === 'string' && thought.body.trim().length > 0,
    `${location}.body is required.`,
  )
  assert(thought.body.length <= 280, `${location}.body exceeds 280 characters.`)
  assert(
    thought.replies === undefined || Array.isArray(thought.replies),
    `${location}.replies must be an array.`,
  )
  assert(
    (thought.replies?.length ?? 0) <= 6,
    `${location}.replies exceeds 6 items.`,
  )
  if (thought.replies)
    validateReplyOrder(thought.replies, `${location}.replies`, thought.date)
}

console.log(
  `Validated ${value.thoughts.length} thoughts and ${value.comments.length} comment threads.`,
)
