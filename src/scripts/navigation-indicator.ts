...
const getTargetGeometry = (activeLink: HTMLAnchorElement): IndicatorGeometry => {
...
const getCurrentGeometry = (nav: HTMLElement, indicator: HTMLElement | null): IndicatorGeometry | null => {
  if (!indicator || indicator.hidden || nav.getClientRects().length === 0) return null;
...
export const createNavigationIndicatorController = (navContainers: HTMLElement[]) => {
  ...
  const read = (targetPath: string): IndicatorState[] =>
    navContainers.map((nav) => {
      ...
      const target = targetLink && nav.getClientRects().length > 0 ? getTargetGeometry(targetLink) : null;
  ...
  const apply = (state: IndicatorState, immediate = false) => {
    ...
    if (indicator.getAttribute('data-nav-ready') !== 'true') indicator.setAttribute('data-nav-ready', 'true');
